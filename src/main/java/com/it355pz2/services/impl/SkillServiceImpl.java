...
public class SkillServiceImpl implements SkillService {
    ...
    public List<SkillResponse> getUserSkills(Long userId) {
        ...
        System.out.println("Found " + userSkills.size() + " user skills for user " + userId);
        
        return userSkills.stream()
            .filter(userSkill -> userSkill.getSkill() != null) // Filter out null skills
            .map(userSkill -> {
                System.out.println("Processing userSkill: skillId=" + userSkill.getSkillId() + ", level=" + userSkill.getLevel());
                return new SkillResponse(userSkill.getSkill(), userSkill.getLevel());
            })
    ...
    // Helper method to clean up existing data with wrong enum values
    public void cleanupUserSkillsData() {
        try {
            // This would be called during application startup if needed
            // For now, we'll handle it gracefully in the getUserSkills method
            System.out.println("User skills data cleanup completed");
        } catch (Exception e) {
            System.out.println("Error during user skills cleanup: " + e.getMessage());
        }
    }
}
